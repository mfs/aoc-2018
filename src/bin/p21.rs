...
fn main() -> Result<(), Error> {
    let  file = BufReader::new(File::open("data/p21.txt")?);
    ...
    loop {
        ...
        const TARGET_REG: usize = 5;
        const TARGET_IP: u64 = 28;

        if registers[IP_REG as usize] == TARGET_IP {
            if rh.is_empty() {
                println!("Part 1: {}", registers[TARGET_REG]);
            }
            if !rh.insert(registers[TARGET_REG]) {
                break;
            }

            last = registers[TARGET_REG];
        }
    }
